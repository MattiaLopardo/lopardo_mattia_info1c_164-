...
from APP_EasyVista_164.categories.gestion_categories_1_forms import FormAjouterCategorie
from APP_EasyVista_164.categories.gestion_categories_1_forms import FormDeleteCategorie
from APP_EasyVista_164.categories.gestion_categories_1_forms import FormUpdateCategorie

"""
    Auteur : OM 2021.03.16
    Définition d'une "route" /categories_afficher
    
    Test : ex : http://127.0.0.1:5005/categories_afficher
    ...
    if request.method == "GET":
        try:
            ...
            raise ExceptionGenresAfficher(f"fichier : {Path(__file__).name}  ;  "
                                          f"{categories_afficher.__name__} ; "
                                          f"{Exception_categorie_afficher}")

    # Envoie la page "HTML" au serveur.
    return render_template("categories/categories_afficher.html", data=data_categorie)
    ...
    if request.method == "POST":
        try:
            if form.validate_on_submit():
                ...
                return redirect(url_for('categories_afficher', order_by='DESC', id_categorie_sel=0))

        except Exception as Exception_categorie_ajouter_wtf:
            raise strsql_insert_categorie(f"fichier : {Path(__file__).name}  ;  "
                                          f"{categories_ajouter_1.__name__} ; "
                                          f"{Exception_categorie_ajouter_wtf}")

    return render_template("categories/categories_ajouter_1.html", form=form)
...
"""
    ...
    Remarque :  Dans le champ "nom_genre_update_wtf" du formulaire "categories/categorie_update_1.html",
                le contrôle de la saisie s'effectue ici en Python.
                On transforme la saisie en minuscules.
                On ne doit pas accepter des valeurs vides, des valeurs avec des chiffres,
                des valeurs avec des caractères qui ne sont pas des lettres.
...
def categorie_update_1():
    ...
    try:
        ...
        if form_update.validate_on_submit():
            ...
            return redirect(url_for('categories_afficher', order_by="ASC", id_categorie_sel=Exception_categorie_ajouter_wtf))
            ...
            # Afficher la valeur sélectionnée dans les champs du formulaire "categorie_update_1.html"
        ...
        raise ExceptionCategorieUpdate(f"fichier : {Path(__file__).name}  ;  "
                                      f"{categorie_update_1.__name__} ; "
                                      f"{Exception_categorie_update}")

    return render_template("categories/categorie_update_1.html", form_update=form_update)
...
"""
    ...
    But : Effacer(delete) un genre qui a été sélectionné dans le formulaire "categories_afficher.html"
    
    Remarque :  Dans le champ "nom_genre_delete_wtf" du formulaire "categories/categorie_delete_1.html",
                le contrôle de la saisie est désactivée. On doit simplement cliquer sur "DELETE"
"""


@app.route("/categorie_delete", methods=['GET', 'POST'])
def categorie_delete_1():
    ...
    try:
        ...
        if request.method == "POST" and form_delete.validate_on_submit():
            ...
            if form_delete.submit_btn_conf_del.data:
                ...
                # le formulaire "categories/categorie_delete_1.html" lorsque le bouton "Etes-vous sur d'effacer ?" est cliqué.
        ...
        if request.method == "GET":
            ...
            # Afficher la valeur sélectionnée dans le champ du formulaire "categorie_delete_1.html"
            form_delete.nom_categorie_delete_wtf.data = data_nom_categorie["id_categorie"]

            # Le bouton pour l'action "DELETE" dans le form. "categorie_delete_1.html" est caché.
            btn_submit_del = False

    except Exception as Exception_categorie_delete_1:
        raise ExceptionCategorieDelete(f"fichier : {Path(__file__).name}  ;  "
                                      f"{categorie_delete_1.__name__} ; "
                                      f"{Exception_categorie_delete_1}")

    return render_template("categories/categorie_delete_1.html",
